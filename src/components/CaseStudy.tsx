...
export default function CaseStudy() {
  ...
  return (
    <section
      id="case-study"
      className="snap-start min-h-dvh relative flex flex-col justify-center px-6 pt-24 pb-28 overflow-hidden [@media(max-height:1000px)]:justify-start"
    ...
    >
      ...
      <motion.p {...rise(0.06)} className="text-[22px] font-extrabold tracking-[-0.02em] text-accent leading-tight mb-5">
        #1 of 88+ games — every single week.
      </motion.p>

      {/* Week-on-week table — 8 rows × 1 col */}
      <motion.div {...rise(0.08)} className="rounded-[12px] border border-line bg-white overflow-hidden shadow-[0_10px_30px_rgba(10,14,26,0.06)]">
        {rows.map((r, i) => (
          <div
            key={r.label}
      ...
      {/* Summary stats — full-width stacked rows */}
      <motion.div {...rise(0.12)} className="flex flex-col gap-2.5 mt-4">
        {summary.map((s) => (
          <div key={s.label} className="flex items-center justify-between rounded-[12px] border border-line bg-white px-5 py-4">
            ...
            <span className="text-[28px] font-extrabold text-accent tnum leading-none">{s.value}</span>
          </div>
        ))}
      </motion.div>
