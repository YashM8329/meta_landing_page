        ...
        <div className="lg:grid lg:grid-cols-2 lg:gap-8">
          ...
          <div className="rounded-[12px] border border-line bg-white p-5 shadow-[0_12px_36px_rgba(10,14,26,0.08)]">
            ...
            <p className="text-[12px] text-ink-faint mt-6 border-t border-line pt-4">
              Based on a <span className="font-semibold text-ink-soft">$65K install cost</span> and <span className="font-semibold text-ink-soft">25 sqm footprint</span>.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2.5 mt-4 lg:mt-0">
            {metrics.map((m) => (
              <div key={m.label} className="flex items-center justify-between rounded-[10px] bg-surface border border-line px-4 py-3.5 lg:flex-1">
                <span className="text-[13px] font-semibold text-ink-soft">{m.label}</span>
                <span className="text-[24px] lg:text-[28px] font-extrabold tnum leading-none text-accent">{m.value}</span>
              </div>
            ))}
          </div>
        </div>
